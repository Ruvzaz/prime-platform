...
import { Calendar, MapPin, Users, Trash2, MoreHorizontal, BarChart3, Radio, Pencil, Globe, Sparkles, GripVertical } from "lucide-react"
...
function SortableEventRow({
  ...
  return (
    <TableRow
      ref={setNodeRef}
      style={style}
      className={`group hover:bg-slate-50/50 border-b-border/30 ${isDragging ? "bg-slate-100 dark:bg-slate-800 shadow-md" : ""}`}
      data-state={selectedIds.includes(event.id) ? "selected" : undefined}
    >
      <TableCell className="w-[40px] px-2 text-center">
        <button
          ...
          {...attributes}
          {...listeners}
          className="p-1.5 rounded text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-grab active:cursor-grabbing transition-colors"
          title="ลากเพื่อจัดลำดับกิจกรรม (Drag to reorder)"
        >
...
export function EventsTable({ initialEvents }: EventsTableProps) {
  ...
  const handleDragEnd = async (dragEvent: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      const result = await updateEventsOrder(reordered.map((e) => e.id))
      if (result.success) {
        toast.success("อัปเดตลำดับกิจกรรมเรียบร้อยแล้ว")
      } else {
            ...
            <TableBody>
              {events.length === 0 ? (
                <TableRow>
                  <TableCell
                    colSpan={8}
                    className="text-center py-32 text-muted-foreground bg-slate-50/10"
                  >
